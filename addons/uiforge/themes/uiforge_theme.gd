...
static func load_named(theme_name: String) -> UIForgeTheme:
	var path := "res://addons/uiforge/themes/%s.theme.json" % theme_name
	if not FileAccess.file_exists(path):
		path = "res://addons/uiforge/themes/dark_fantasy.theme.json"
	var file := FileAccess.open(path, FileAccess.READ)
	if file == null:
		return UIForgeTheme.new(_fallback_data())
	var parser := JSON.new()
	parser.parse(file.get_as_text())
	file.close()
	return UIForgeTheme.new(parser.data if parser.data is Dictionary else _fallback_data())
