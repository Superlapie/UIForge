@tool
class_name UIForgePropertyInspector
extends ScrollContainer

signal apply_requested(changes: Dictionary)

const RESOURCE_FIELD_SCRIPT := preload("res://addons/uiforge/inspectors/uiforge_resource_field.gd")
...
func _rebuild() -> void:
	...
	body.add_child(hint)
	var last_category := ""
	for spec in UIForgePropertyCatalog.definitions(str(current_node.get("type", "Control"))):
...
func _native_specs(node_type: String) -> Array[Dictionary]:
	...
	for property_info in UIForgePropertyCatalog.native_properties_for(node_type):
