class_name AetherPropertyCatalog
extends RefCounted
...
static func definitions(node_type: String) -> Array[Dictionary]:
	var native := AetherComponentLibrary.native_type(node_type)
...
static func capability_groups() -> Dictionary:
	...
	for node_type in AetherTypes.NATIVE_TYPES + AetherTypes.COMPONENT_TYPES:
