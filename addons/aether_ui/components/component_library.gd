class_name AetherComponentLibrary
extends RefCounted
...
static func materialize(node: Dictionary, custom_definitions: Dictionary = {}) -> Dictionary:
	...
	if type == "ComponentInstance":
		...
		result["type"] = str(result.get("base_type", definition.get("native_type", AetherComponentLibrary.native_type(str(definition.get("base", "Control"))))))
