class_name AetherTemplates
extends RefCounted
## Shared populated screen catalog for the editor and CLI.

static func catalog() -> Dictionary:
	var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string("res://addons/aether_ui/components/templates.json"))
	return parsed if parsed is Dictionary else {}

static func create(template_name: String, document_name: String) -> Dictionary:
	if template_name in ["blank", "window", "modal"]:
		return {"document": AetherSerializer.create_default(document_name), "errors": []}
	var entries := catalog()
	...
	var result := AetherSerializer.load_document(str(entries[template_name].path))
