...
func validate(input: AetherDocument, theme_override: AetherTheme = null) -> Dictionary:
	document = input
	theme = theme_override if theme_override != null else AetherTheme.from_document(input)
	diagnostics = []
	ids = {}
	names = {}
	if int(document.data.get("schema_version", 0)) != AetherTypes.SCHEMA_VERSION:
		_add("error", "SCHEMA_VERSION_UNSUPPORTED", "Document schema_version must be %d." % AetherTypes.SCHEMA_VERSION, "", "Run a migration or update the document schema.")
...
func _validate_node(node: Dictionary, parent_id: String, viewport: Dictionary) -> void:
	...
	if node_type.is_empty():
		...
		if not AetherComponentLibrary.has(component_name) and not custom_components.has(component_name):
	...
	if node_type in AetherTypes.INTERACTIVE_TYPES and size.size() >= 2 and (float(size[0]) < 32.0 or float(size[1]) < 32.0):
...
func _validate_node_properties(node: Dictionary, node_id: String) -> void:
	...
	if properties.has("texture_region"):
		...
		if not valid or not str(properties.get("texture", "")).begins_with("res://") or AetherComponentLibrary.native_type(str(node.get("type", ""))) not in ["TextureRect", "TextureButton", "NinePatchRect"]:
...
func _validate_sibling_overlaps(children: Array, parent_id: String, parent_type: String) -> void:
	if parent_type in AetherTypes.CONTAINER_TYPES or children.size() < 2:
		return
...
func _is_interactive(node: Dictionary) -> bool:
	return str(node.get("type", "")) in AetherTypes.INTERACTIVE_TYPES or str(node.get("action", "")).is_empty() == false
...
func _known_type(type_name: String) -> bool:
	return type_name in AetherTypes.NATIVE_TYPES or type_name in AetherTypes.COMPONENT_TYPES or type_name == "ComponentInstance"
