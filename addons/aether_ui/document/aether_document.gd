...
func _init(initial_data: Dictionary = {}) -> void:
	...
	if not data.has("schema_version"):
		data["schema_version"] = AetherTypes.SCHEMA_VERSION
	if not data.has("metadata"):
		data["metadata"] = {}

static func from_dict(value: Dictionary, path: String = "") -> AetherDocument:
	var document := AetherDocument.new(value)
