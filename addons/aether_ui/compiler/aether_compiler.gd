class_name AetherCompiler
extends RefCounted

var document: AetherDocument
var theme: AetherTheme
...
static func document_for_preview_state(input: AetherDocument, state: String) -> AetherDocument:
	var result := AetherDocument.from_dict(input.data, input.source_path)
	var normalized := state.to_lower()
	if normalized in ["", "normal"]:
		return result
	var active_state := "focused" if normalized == "focus" else normalized
	var theme := AetherTheme.from_document(result)
	_apply_preview_state(result.root(), active_state, theme)
	return result

static func _apply_preview_state(node: Dictionary, state: String, theme: AetherTheme) -> void:
	...
	if state_override.is_empty() and state != "normal":
		...
		var themed_style := theme.style(str(node.get("style", AetherComponentLibrary.style_name(semantic_type))))
	...
	if not state_override.is_empty():
		var base_style: Dictionary = theme.style(AetherComponentLibrary.style_name(str(node.get("type", "Control"))))
...
func compile_file(input_path: String, output_path: String = "") -> Dictionary:
	var loaded := AetherSerializer.load_document(input_path)
...
func compile_document(input: AetherDocument, output_path: String, original_source: String = "") -> Dictionary:
	document = input
	source_path = original_source if not original_source.is_empty() else input.source_path
	theme = AetherTheme.from_document(input)
	custom_components = input.data.get("components", {})
	subresource_lines = []
	ext_resources = {}
	style_ids = {}
	var validation := AetherValidator.new().validate(input, theme)
	var errors: Array = []
	...
	lines.append("; %s" % AetherTypes.GENERATED_MARKER)
...
func _materialize_node(node: Dictionary) -> Dictionary:
	var result := AetherComponentLibrary.materialize(node, custom_components)
...
func _emit_node(node: Dictionary, parent_path: String, lines: Array[String], is_root: bool = false) -> void:
	...
	var native_type := AetherComponentLibrary.native_type(str(resolved.get("type", "Control")))
...
func _children_for(node: Dictionary) -> Array:
	...
	if AetherComponentLibrary.native_type(node_type) == "Panel":
		var visual := theme.style(str(node.get("style", AetherComponentLibrary.style_name(node_type))))
		if node.get("style") is Dictionary:
			visual = theme.style(AetherComponentLibrary.style_name(node_type))
...
func _emit_styles(node: Dictionary, native_type: String, node_id: String, lines: Array[String]) -> void:
	...
	if native_type not in ["Panel", "PanelContainer", "Button", "TextureButton", "CheckBox", "CheckButton", "OptionButton", "MenuButton", "LinkButton", "LineEdit", "ProgressBar", "TabBar", "ScrollContainer"]:
		return
	var node_type := str(node.get("type", "Control"))
	var style_name := str(node.get("style", AetherComponentLibrary.style_name(node_type)))
	if node.get("style") is Dictionary:
		style_name = AetherComponentLibrary.style_name(node_type)
	...
	if native_type == "ProgressBar":
		...
		var default_fill_style := str(AetherComponentLibrary.definitions().get(node_type, {}).get("fill_style", "button_primary"))
...
func _native_type(type_name: String) -> String:
	return AetherComponentLibrary.native_type(type_name)
