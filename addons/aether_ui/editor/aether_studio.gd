@tool
class_name AetherStudio
extends VBoxContainer

const CANVAS_SCRIPT := preload("res://addons/aether_ui/canvas/aether_canvas.gd")
const NINE_SLICE_SCRIPT := preload("res://addons/aether_ui/editor/aether_nine_slice_editor.gd")
const INSPECTOR_SCRIPT := preload("res://addons/aether_ui/inspectors/aether_property_inspector.gd")
const ASSET_LIST_SCRIPT := preload("res://addons/aether_ui/editor/aether_asset_list.gd")
const HIERARCHY_TREE_SCRIPT := preload("res://addons/aether_ui/editor/aether_hierarchy_tree.gd")
...
var tree: AetherHierarchyTree
var canvas: AetherCanvas
var inspector: VBoxContainer
var property_inspector: AetherPropertyInspector
...
var asset_list: AetherAssetList
...
func _ready() -> void:
	if get_child_count() == 0:
		_build_ui()
	if document == null:
		set_document(AetherSerializer.create_default("untitled"), "")
...
func _build_toolbar() -> void:
	...
	for state in AetherTypes.STATES:
	...
	for preset in AetherTypes.VIEWPORT_PRESETS:
...
func _build_bottom_panel() -> void:
	...
	for component_name in AetherComponentLibrary.definitions().keys():
...
func _on_asset_drop(path: String, target_node_id: String, design_position: Vector2) -> void:
	...
	if not target_node.is_empty() and target_node_id != root_id and str(target_node.get("type", "")) in AetherTypes.CONTAINER_TYPES:
		...
		if not selected_parent.is_empty() and str(selected_parent.get("type", "")) in AetherTypes.CONTAINER_TYPES:
...
func _load_document_path(path: String) -> void:
	var loaded := AetherSerializer.load_document(path)
...
func set_document(value: AetherDocument, path: String = "") -> void:
	document = value
	document_path = path
	dirty = false
	if canvas != null:
...
func _on_inspector_apply(changes: Dictionary) -> void:
	...
	for path in changes:
		...
		if str(path).begins_with(AetherPropertyInspector.NATIVE_PREFIX):
			var native_name := str(path).trim_prefix(AetherPropertyInspector.NATIVE_PREFIX)
			var properties: Dictionary = node.get("properties", {})
...
func _create_new_document() -> void:
	if template_dialog == null:
		...
		var entries := AetherTemplates.catalog()
		for key in entries:
...
func _create_selected_template() -> void:
	...
	var created := AetherTemplates.create(key, "untitled" if key == "blank" else key)
...
func _on_save() -> void:
	if document_path.is_empty():
		...
		file_dialog.popup_centered_ratio(0.7)
		return
	var result := AetherSerializer.save_document(document, document_path)
	if result.success:
		dirty = false
		_update_status()
	else:
...
func _on_file_selected(path: String) -> void:
	if dialog_mode == "reference":
		...
		return
	if dialog_mode == "save":
		var result := AetherSerializer.save_document(document, path)
		if result.success:
			document_path = path
			dirty = false
			_update_status()
...
func _on_validate() -> void:
	if document == null:
		return
	var result := AetherValidator.new().validate(document)
...
func _on_build() -> void:
	...
	var result := AetherCompiler.new().compile_document(document, output, document_path)
	if result.success:
...
func _on_preview() -> void:
	...
	var preview_document := AetherCompiler.document_for_preview_state(document, state)
	var preview_path := "user://aether_preview_%s.tscn" % document.document_name()
	var result := AetherCompiler.new().compile_document(preview_document, preview_path, document_path)
