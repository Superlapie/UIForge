...
func _dispatch() -> Dictionary:
	...
	match command:
		"capabilities":
			return {"success": true, "capabilities": AetherCapabilities.as_dict()}
		"new":
...
func _load(path: String) -> Dictionary:
	return AetherSerializer.load_document(path)
...
func _new_document() -> Dictionary:
	...
	var created := AetherTemplates.create(template, output.get_file().trim_suffix(".ui.json"))
	if created.get("document") == null:
		return {"success": false, "errors": created.errors}
	var document: AetherDocument = created.document
	var saved := AetherSerializer.save_document(document, output)
...
func _validate_document() -> Dictionary:
	...
	var validation := AetherValidator.new().validate(loaded["document"])
...
func _inspect_document() -> Dictionary:
	...
	if loaded.get("document") == null:
		return {"success": false, "errors": loaded.get("errors", [])}
	var document: AetherDocument = loaded["document"]
	var scope := str(args[2]) if args.size() > 2 else "tree"
	match scope:
		"document":
			return {"success": true, "document": {"schema_version": document.data.get("schema_version", AetherTypes.SCHEMA_VERSION), "name": document.document_name(), "source": document.source_path, "viewport": document.data.get("viewport", {}), "theme": document.data.get("theme", "dark_fantasy"), "root_id": str(document.root().get("id", "")), "root_type": str(document.root().get("type", "")), "node_count": document.all_nodes().size(), "components": document.data.get("components", {}).keys(), "metadata": document.data.get("metadata", {}), "reference": document.data.get("reference", {})}}
		"tree":
			return AetherDocumentOperations.inspect_tree(document)
		"node":
			if args.size() < 4:
				return {"success": false, "errors": [{"code": "USAGE", "message": "Usage: ui inspect <document.ui.json> node <node_id>"}]}
			return AetherDocumentOperations.get_node(document, str(args[3]))
		"tokens":
			return {"success": true, "theme": document.data.get("theme", "dark_fantasy"), "tokens": AetherTheme.from_document(document).tokens}
		"components":
			return {"success": true, "components": AetherComponentLibrary.definitions(), "custom": document.data.get("components", {})}
		"diagnostics":
			var validation := AetherValidator.new().validate(document)
			return {"success": validation.success, "diagnostics": validation.diagnostics}
		_:
			if not document.find_node(scope).is_empty():
				return AetherDocumentOperations.get_node(document, scope)
...
func _set_value() -> Dictionary:
	...
	if loaded.get("document") == null:
		return {"success": false, "errors": loaded.get("errors", [])}
	var document: AetherDocument = loaded["document"]
	var operation := AetherDocumentOperations.set_value(document, str(args[2]), str(args[3]), str(args[4]))
	if operation.success:
		var saved := AetherSerializer.save_document(document, str(args[1]))
...
func _add_value() -> Dictionary:
	...
	if loaded.get("document") == null:
		return {"success": false, "errors": loaded.get("errors", [])}
	var document: AetherDocument = loaded["document"]
	var operation := AetherDocumentOperations.add_value(document, str(args[2]), str(args[3]))
	if operation.success:
		operation["saved"] = AetherSerializer.save_document(document, str(args[1])).success
	return operation
...
func _delete_value() -> Dictionary:
	...
	if loaded.get("document") == null:
		return {"success": false, "errors": loaded.get("errors", [])}
	var document: AetherDocument = loaded["document"]
	var operation := AetherDocumentOperations.delete_value(document, str(args[2]))
	if operation.success:
		operation["saved"] = AetherSerializer.save_document(document, str(args[1])).success
	return operation
...
func _move_value() -> Dictionary:
	...
	var operation := AetherDocumentOperations.move_value(document, str(args[2]), str(args[3]), index)
	if operation.success:
		operation["saved"] = AetherSerializer.save_document(document, str(args[1])).success
	return operation
...
func _duplicate_value() -> Dictionary:
	...
	if loaded.get("document") == null:
		return {"success": false, "errors": loaded.get("errors", [])}
	var document: AetherDocument = loaded["document"]
	var operation := AetherDocumentOperations.duplicate_value(document, str(args[2]), str(args[3]))
	if operation.success:
		operation["saved"] = AetherSerializer.save_document(document, str(args[1])).success
	return operation
...
func _build_document() -> Dictionary:
	...
	return AetherCompiler.new().compile_file(source, output)
...
func _build_all() -> Dictionary:
	...
	for source_file in source_files:
		...
		var result := AetherCompiler.new().compile_file("%s/%s" % [source_dir, source_file], output)
		if result.success:
			built.append(result)
		else:
...
func _render_document() -> Dictionary:
	...
	var loaded := AetherSerializer.load_document(source)
	if loaded.get("document") == null:
		return {"success": false, "errors": loaded.get("errors", [])}
	var render_document: AetherDocument = AetherCompiler.document_for_preview_state(loaded["document"], state)
	var compiled := AetherCompiler.new().compile_document(render_document, generated_scene, source)
