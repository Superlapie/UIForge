...
func refresh_native_preview() -> void:
	...
	var preview := AetherCompiler.document_for_preview_state(document, preview_state)
	var path := "user://aether_canvas_%s_%s.tscn" % [OS.get_process_id(), get_instance_id()]
	var compiled := AetherCompiler.new().compile_document(preview, path)
...
func set_document(value: AetherDocument) -> void:
	document = value
	native_signature = ""
	native_preview_valid = false
	native_controls.clear()
...
func _draw_children(node: Dictionary) -> Array:
	...
	children.append_array(AetherCompiler.decoration_children(node))
	return children
...
func _fill_for(node: Dictionary) -> Color:
	...
	if document != null:
		var theme := AetherTheme.from_document(document)
		var style_value: Variant = node.get("style", AetherComponentLibrary.style_name(node_type))
...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	if not target_node.is_empty() and str(target_node.get("type", "")) in AetherTypes.CONTAINER_TYPES:
