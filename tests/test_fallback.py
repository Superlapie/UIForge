...
class UIForgeFallbackTests(unittest.TestCase):
    def test_atlas_templates_and_regions(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            for key in templates:
                ...
                result, code = run("new", key, str(source), "--allow-outside-project")
                self.assertEqual(code, 0, (key, result))
                result, code = run("validate", str(source))
                self.assertEqual(code, 0, (key, result))
                result, code = run("build", str(source), str(source.with_suffix(".tscn")), "--allow-outside-project")
                self.assertEqual(code, 0, (key, result))
            ...
            result, code = run("new", "unknown", str(missing), "--allow-outside-project")
    ...
    def test_all_examples_compile_to_generated_scenes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            result, code = run("build-all", str(ROOT / "examples/specs"), directory, "--allow-outside-project")
            ...
            for scene in Path(directory).glob("*.tscn"):
                ...
                self.assertIn("metadata/uiforge_id", text)
    ...
    def test_component_instance_is_materialized_by_fallback_compiler(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            source.write_text(json.dumps(document), encoding="utf-8")
            result, code = run("build", str(source), str(output), "--allow-outside-project")
            self.assertEqual(code, 0, result)
    ...
    def test_full_property_fixture_is_valid_and_buildable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("metadata/uiforge_transitions", scene)
            self.assertIn("metadata/uiforge_effects", scene)
